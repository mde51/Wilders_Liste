...
class Liste extends Component {
  constructor(props) {
    super(props);
    this.state = { wilders: [], email: "", liste: [], modification: [] };
  }
  componentDidMount() {
  ...
  modificationMail(e) {
    e.preventDefault();
    axios.delete(`/modification/${this.state.email}`).then(response => {
        ...
        <Row>
          <Col>
            <Form onSubmit={this.handleSubmit}>
              <h2 className="text-secondary font-weight-bold mt-5" >Rechercher un Wilder par son email pour le supprimer</h2>
            ...
            {/* je test mon state ? si state = "quelque choses" , j'affiche <Row> FICHE </Row> : si non , j'affiche logo    {this.state.morning?<Page2/>:<Page3/>} */}
            {this.state.email ?
            <Row>
              <Col className="d-flex justify-content-center mt-5">
                <div className="text-center ">
                  ...
                  <Button
                    className="btn bg-primary float-right mt-2"
                    onClick={e => this.modificationMail(e)}
                    type="submit"
                    value="submit"
                  >
                    Pour mofifier le Wilder Valider !!!
                  </Button>
                </div>
              </Col>
            </Row>
